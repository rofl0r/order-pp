...
#define ORDER_PP_8FOR_EACH_IN_RANGE(P,i0,i1,...) (,,ORDER_PP_FX(8FOR_EACH_IN_RANGE_B,ORDER_PP_NUM_BOP(,LESS,P##i0,P##i1)(,(,8INC,P##i0,ORDER_PP_NUM_BOP(,MINUS,P##i1,P##i0)),(,8DEC,ORDER_PP_NUM_UOP(,DEC,P##i0),ORDER_PP_NUM_BOP(,MINUS,P##i0,P##i1)))),P##__VA_ARGS__)
#define ORDER_PP_8FOR_EACH_IN_RANGE_B(P,dir,i,n) ORDER_PP_SELECT_4(ORDER_PP_IS_EDIBLE(,P##i),ORDER_PP_IS_EDIBLE(,P##n))(,ORDER_PP_OPEN(,8FOR_EACH_IN_RANGE_NAT_##dir,P##i,P##n),ORDER_PP_OPEN(,8FOR_EACH_IN_RANGE_NAT_##dir,P##i,ORDER_PP_LIT_TO_NAT(,P##n)),ORDER_PP_OPEN(,8FOR_EACH_IN_RANGE_NAT_##dir,ORDER_PP_LIT_TO_NAT(,P##i),P##n),8FOR_EACH_IN_RANGE_LIT_##dir,P##i,P##n)
