...
//
// \begin{enumerate}
// \item Try preprocessing this example with different values of
//       `EXP_DELAY_N'. What is the maximum number you can use
//       before the preprocessor runs out of memory or you get tired
//       of waiting?
//
// \item Convince yourself that `exp_delay(n)' can be evaluated in
//       $O(n)$ space. For extra credit, write a formal proof.
//
// \item Modify this example so that instead of expanding to
//       nothing, each time `exp_delay' is called, it outputs a
//       single `+'. How many `+' tokens you get when `EXP_DELAY_N'
//       is `10' or `50' (assuming the program would actually
//       finish)?
// \end{enumerate}
