...
// function. First we'll define the \code{linear_fib_iter} function:
// \begin{verbatim}
...
// \end{verbatim}
// \code{linear_fib_iter(n,i,j)} recursively performs \code{n} steps
// of Fibonacci iteration, starting with the values \code{i} and
// \code{j}. During recursion, \code{i} and \code{j} act as
// accumulators. Then we'll define the function \code{linear_fib},
// which is just a simple driver for the Fibonacci iterator
// function.
// \begin{verbatim}
...
// we'll define the driver function \code{8linear_fib}:
// \begin{verbatim}
#define ORDER_PP_DEF_8linear_fib                \
ORDER_PP_FN(8fn(8N,                             \
                8linear_fib_iter(8N, 0, 1)))
// \end{verbatim}
// As you can see, the Order function \code{8linear_fib} is
// structurally very similar to the previously defined C function
// \code{linear_fib}. We can refer to the \code{8linear_fib_iter}
// function without having to declare it first. Then we'll define
// the Order function \code{8linear_fib_iter}, which is structurally
// very similar to the C function \code{linear_fib_iter}:
// \begin{verbatim}
...
ORDER_PP_FN(8fn(8N, 8I, 8J,                             \
                8if(8is_0(8N),                          \
                    ...
                    8linear_fib_iter(8dec(8N),          \
                                     ...
                                     8add(8I, 8J)))))
// \end{verbatim}
// The Order function \code{8linear_fib} can now be used to compute
// Fibonacci numbers.
//
...
//       correctly.
//
// \item Is the \code{8linear_fib_iter} function tail recursive? How
//       much space does the evaluation of
//       \code{8linear_fib_iter(n)} use in terms of \code{n}?
//
// \item Modify this example so that it prints the first 50
//       Fibonacci numbers computed using the \code{8linear_fib}
//       function. (Hint: Use the function \code{8for_each_in_range}
//       and the operator \code{8emit}.)
// \end{enumerate}
