...
// meaning that only one string containing a particular sequence of
// characters is created. Interned strings can be compared for equality
...
// interned string. This function does not allocate a new string, but the
// the same pointer will be returned for all strings that have the same
...
// `beyond'. Note that the pointers `first' and `beyond' must point to the
// same string.
...
// parameter. Note that you just can't use `0' instead of `str_end',
// because the literal `0' is an `int'.
extern str_type str_cat(str_type s0, ...);

// `str_skip_spaces(&str)' advanced the string `str' past the whitespace
...
// will be non-zero and the `str' string will be advanced by the length of
// the `maybe_prefix' string. Otherwise the return value will be `0' and
// `str' will not be modified. Note that whitespace is significant to this
// function.
